...
public class PaperParcelAutoValueExtension
  ...
  private static final TypeName PAPER_PARCELS =
      ClassName.get("nz.bradcampbell.paperparcel", "PaperParcels");
  ...
  private static boolean needsContentDescriptor(Context context) {
    ...
    for (ExecutableElement element : MoreElements.getLocalAndInheritedMethods(
      ...
      if (element.getSimpleName().contentEquals("describeContents") && MoreTypes.isTypeOf(int.class,
          element.getReturnType()) && element.getParameters().isEmpty() && !element.getModifiers()
  ...
  private static ExecutableElement findWriteToParcel(Context context) {
    ...
    for (ExecutableElement element : MoreElements.getLocalAndInheritedMethods(
        context.autoValueClass(), env.getElementUtils())) {
      if (element.getSimpleName().contentEquals("writeToParcel") && MoreTypes.isTypeOf(void.class,
          element.getReturnType()) && !element.getModifiers().contains(Modifier.ABSTRACT)) {
        List<? extends VariableElement> parameters = element.getParameters();
        if (parameters.size() == 2 && env.getTypeUtils()
            .isSameType(parcel, parameters.get(0).asType()) && MoreTypes.isTypeOf(int.class,
  ...
  private static VariableElement findCreator(Context context) {
    ...
    TypeMirror creatorType =
        typeUtils.erasure(elementUtils.getTypeElement("android.os.Parcelable.Creator").asType());
    ...
    for (VariableElement field : ElementFilter.fieldsIn(members)) {
      if (field.getSimpleName().contentEquals("CREATOR") && typeUtils.isSameType(creatorType,
          typeUtils.erasure(field.asType())) && field.getModifiers().contains(Modifier.STATIC)) {
        return field;
      }
    }
    return null;
  ...
  @Override public boolean applicable(Context context) {
    ...
    TypeMirror parcelable =
        processingEnv.getElementUtils().getTypeElement("android.os.Parcelable").asType();
      ...
      context.processingEnvironment()
          ...
          .printMessage(Diagnostic.Kind.ERROR,
              "Manual implementation of a static Parcelable.Creator<T> CREATOR field found when processing "
                  + autoValueClass.toString()
                  + ". Remove this so PaperParcel can automatically generate the implementation "
                  + "for you.", creator);
    }
  ...
  private FieldSpec generateCreator(TypeName thisClass, FieldSpec classLoader) {
    ...
    CodeBlock.Builder initializer = CodeBlock.builder()
        ...
        .beginControlFlow("@$T public $T createFromParcel($T in)", ClassName.get(Override.class),
            thisClass, PARCEL);
    initializer.addStatement("return $T.unwrap(in.readParcelable($N))", PAPER_PARCELS, classLoader);
    initializer.endControlFlow()
        .beginControlFlow("@$T public $T[] newArray($T size)", ClassName.get(Override.class),
            thisClass, int.class)
  ...
  private FieldSpec generateClassLoaderField(TypeName className) {
    ...
    return FieldSpec.builder(ClassLoader.class, "CLASS_LOADER", Modifier.PRIVATE, FINAL,
