...
public class ListProperty extends Property {
  ...
  @Override protected CodeBlock readFromParcelInner(CodeBlock.Builder block, ParameterSpec in,
    ...
    if (isInterface) {
      block.addStatement("$T $N = new $T<$T>($N)", typeName, listName, ArrayList.class,
          parameterTypeName, listSize);
    } else {
    ...
    block.beginControlFlow("for (int $N = 0; $N < $N; $N++)", indexName, indexName, listSize,
        indexName);
  ...
  @Override protected void writeToParcelInner(CodeBlock.Builder block, ParameterSpec dest,
    ...
    block.addStatement("$T $N = $L.get($N)", parameterTypeName, parameterItemName, sourceLiteral,
        indexName);
