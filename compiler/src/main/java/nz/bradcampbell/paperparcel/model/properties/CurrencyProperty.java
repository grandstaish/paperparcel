...
public class CurrencyProperty extends Property {
  public CurrencyProperty(boolean isNullable, TypeName typeName, String name) {
  ...
  @Override protected CodeBlock readFromParcelInner(CodeBlock.Builder block, ParameterSpec in,
    ...
    return CodeBlock.of("$T.getInstance($N.readString())", Currency.class, in);
  }
  ...
  @Override protected void writeToParcelInner(CodeBlock.Builder block, ParameterSpec dest,
      ...
      Set<String> scopedVariableNames) {
    block.addStatement("$N.writeString($L.getCurrencyCode())", dest, sourceLiteral);
  }
}
