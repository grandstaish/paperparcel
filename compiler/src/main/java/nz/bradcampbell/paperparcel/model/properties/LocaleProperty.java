...
public class LocaleProperty extends Property {
  public LocaleProperty(boolean isNullable, TypeName typeName, String name) {
  ...
  @Override protected CodeBlock readFromParcelInner(CodeBlock.Builder block, ParameterSpec in,
    ...
    return CodeBlock.of("$T.forLanguageTag($N.readString())", Locale.class, in);
  }
  ...
  @Override protected void writeToParcelInner(CodeBlock.Builder block, ParameterSpec dest,
      ...
      Set<String> scopedVariableNames) {
    block.addStatement("$N.writeString($L.toLanguageTag())", dest, sourceLiteral);
  }
}
