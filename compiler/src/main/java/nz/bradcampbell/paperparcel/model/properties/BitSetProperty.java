...
public class BitSetProperty extends Property {
  public BitSetProperty(boolean isNullable, TypeName typeName, String name) {
  ...
  @Override protected CodeBlock readFromParcelInner(CodeBlock.Builder block, ParameterSpec in,
    ...
    return CodeBlock.of("$T.valueOf($N.createLongArray())", BitSet.class, in);
  }
  ...
  @Override protected void writeToParcelInner(CodeBlock.Builder block, ParameterSpec dest,
      ...
      Set<String> scopedVariableNames) {
    block.addStatement("$N.writeLongArray($L.toLongArray())", dest, sourceLiteral);
  }
}
