...
public class BigDecimalTests {
  ...
  @Test public void bigDecimalTest() throws Exception {
    ...
    JavaFileObject expectedSource =
        JavaFileObjects.forSourceString("test/Test$$Wrapper", Joiner.on('\n').join(
            ...
            "      BigDecimal outChild = null;",
            "      if (in.readInt() == 0) {",
            "        BigInteger childUnscaled = new BigInteger(in.createByteArray());",
            "        outChild = new BigDecimal(childUnscaled, in.readInt());",
            "      }",
            ...
            "    BigDecimal child = this.data.getChild();",
            "    if (child == null) {",
            "      dest.writeInt(1);",
            "    } else {",
            "      dest.writeInt(0);",
            "      dest.writeByteArray(child.unscaledValue().toByteArray());",
            "      dest.writeInt(child.scale());",
            "    }",
            "  }",
            "}"
        ));
