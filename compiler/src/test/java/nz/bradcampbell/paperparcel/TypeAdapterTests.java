...
public class TypeAdapterTests {
  ...
  @Test public void dateTypeAdapterTest() throws Exception {
    JavaFileObject source = JavaFileObjects.forSourceString("test.Test", Joiner.on('\n')
        .join("package test;", "import test.DateTypeAdapter;",
            "import nz.bradcampbell.paperparcel.TypeAdapters;",
            "import nz.bradcampbell.paperparcel.PaperParcel;", "import java.util.Date;",
            "@PaperParcel", "@TypeAdapters(DateTypeAdapter.class)", "public final class Test {",
            "private final Date child;", "public Test(Date child) {", "this.child = child;", "}",
            "public Date getChild() {", "return this.child;", "}", "}"));

    JavaFileObject typeAdapter = JavaFileObjects.forSourceString("test.DateTypeAdapter",
        Joiner.on('\n')
            .join("package test;", "import nz.bradcampbell.paperparcel.TypeAdapter;",
                "import java.util.Date;", "import android.os.Parcel;",
                "public class DateTypeAdapter implements TypeAdapter<Date> {",
                "public Date readFromParcel(Parcel in) {", "return new Date(in.readLong());", "}",
    ...
    JavaFileObject expectedSource = JavaFileObjects.forSourceString("test/TestParcel",
        Joiner.on('\n')
            .join("package test;", "import android.os.Parcel;", "import android.os.Parcelable;",
                ...
                "DateTypeAdapter dateTypeAdapter = new DateTypeAdapter();", "Date outChild = null;",
                "if (in.readInt() == 0) {", "outChild = dateTypeAdapter.readFromParcel(in);", "}",
                ...
                "Date child = this.data.getChild();", "if (child == null) {", "dest.writeInt(1);",
