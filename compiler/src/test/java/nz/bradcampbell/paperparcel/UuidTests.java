...
public class UuidTests {
  ...
  @Test public void uuidTest() throws Exception {
    ...
    JavaFileObject expectedSource =
        JavaFileObjects.forSourceString("test/Test$$Wrapper", Joiner.on('\n').join(
            ...
            "      UUID outChild = null;",
            "      if (in.readInt() == 0) {",
            "        outChild = new UUID(in.readLong(), in.readLong());",
            "      }",
            ...
            "    UUID child = this.data.getChild();",
            "    if (child == null) {",
            "      dest.writeInt(1);",
            "    } else {",
            "      dest.writeInt(0);",
            "      dest.writeLong(child.getMostSignificantBits());",
            "      dest.writeLong(child.getLeastSignificantBits());",
            "    }",
            "  }",
            "}"
        ));
