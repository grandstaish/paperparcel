...
public class LocaleTests {
  ...
  @Test public void localeTest() throws Exception {
    JavaFileObject source =
        JavaFileObjects.forSourceString("test.Test", Joiner.on('\n').join(
            ...
            "  private final Locale child;",
            "  public Test(Locale child) {",
            "    this.child = child;",
            "  }",
            "  public Locale getChild() {",
            "    return this.child;",
            "  }",
            "}"
        ));
    ...
    JavaFileObject expectedSource =
        JavaFileObjects.forSourceString("test/Test$$Wrapper", Joiner.on('\n').join(
            ...
            "      Locale outChild = null;",
            "      if (in.readInt() == 0) {",
            "        outChild = Locale.forLanguageTag(in.readString());",
            "      }",
            ...
            "    Locale child = this.data.getChild();",
            "    if (child == null) {",
            "      dest.writeInt(1);",
            "    } else {",
            "      dest.writeInt(0);",
            "      dest.writeString(child.toLanguageTag());",
            "    }",
            "  }",
            "}"
        ));
