  ...
  @Test public void bundleTest() throws Exception {
    JavaFileObject source = JavaFileObjects.forSourceString("test.Test", Joiner.on('\n')
        .join("package test;", "import android.os.Bundle;",
            ...
            "public final class Test {", "private final Bundle child;",
            "public Test(Bundle child) {", "this.child = child;", "}", "public Bundle getChild() {",
            "return this.child;", "}", "}"));
    ...
    JavaFileObject expectedSource = JavaFileObjects.forSourceString("test/TestParcel",
        Joiner.on('\n')
            .join("package test;", "import android.os.Bundle;", "import android.os.Parcel;",
                ...
                "Bundle outChild = null;", "if (in.readInt() == 0) {",
                "outChild = in.readBundle(CLASS_LOADER);", "}", "Test data = new Test(outChild);",
                ...
                "Bundle child = this.data.getChild();", "if (child == null) {", "dest.writeInt(1);",
                "} else {", "dest.writeInt(0);", "dest.writeBundle(child);", "}", "}", "}"));
