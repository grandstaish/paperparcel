  ...
  @Test public void longArrayTest() throws Exception {
    JavaFileObject source = JavaFileObjects.forSourceString("test.Test", Joiner.on('\n')
        .join("package test;", "import nz.bradcampbell.paperparcel.PaperParcel;", "@PaperParcel",
            "public final class Test {", "private final long[] child;",
            "public Test(long[] child) {", "this.child = child;", "}", "public long[] getChild() {",
            "return this.child;", "}", "}"));
    ...
    JavaFileObject expectedSource = JavaFileObjects.forSourceString("test/TestParcel",
        Joiner.on('\n')
            .join("package test;", "import android.os.Parcel;", "import android.os.Parcelable;",
                ...
                "long[] outChild = null;", "if (in.readInt() == 0) {",
                "outChild = in.createLongArray();", "}", "Test data = new Test(outChild);",
                ...
                "long[] child = this.data.getChild();", "if (child == null) {", "dest.writeInt(1);",
                "} else {", "dest.writeInt(0);", "dest.writeLongArray(child);", "}", "}", "}"));
