...
public class ParcelableTests {
  ...
  @Test public void bitmapTest() throws Exception {
    JavaFileObject source = JavaFileObjects.forSourceString("test.Test", Joiner.on('\n')
        .join("package test;", "import android.graphics.Bitmap;", "import android.os.Parcelable;",
            "import nz.bradcampbell.paperparcel.PaperParcel;", "@PaperParcel",
            "public final class Test {", "private final Bitmap child;",
            "public Test(Bitmap child) {", "this.child = child;", "}", "public Bitmap getChild() {",
            "return this.child;", "}", "}"));
    ...
    JavaFileObject expectedSource = JavaFileObjects.forSourceString("test/TestParcel",
        Joiner.on('\n')
            .join("package test;", "import android.graphics.Bitmap;", "import android.os.Parcel;",
                ...
                "Bitmap child = this.data.getChild();", "if (child == null) {", "dest.writeInt(1);",
                "} else {", "dest.writeInt(0);", "child.writeToParcel(dest, flags);", "}", "}",
                "}"));
  ...
  @Test public void extendsParcelableTest() throws Exception {
    JavaFileObject dataClassRoot = JavaFileObjects.forSourceString("test.Root", Joiner.on('\n')
        .join("package test;", "import nz.bradcampbell.paperparcel.PaperParcel;",
            "import java.util.List;", "@PaperParcel", "public final class Root {",
            "private final Child child;", "public Root(Child child) {", "this.child = child;", "}",
            "public Child getChild() {", "return this.child;", "}", "}"));

    JavaFileObject customParcelable = JavaFileObjects.forSourceString("test.CustomParcelable",
        Joiner.on('\n')
            .join("package test;", "import android.os.Parcelable;",
    ...
    JavaFileObject dataClassChild = JavaFileObjects.forSourceString("test.Child", Joiner.on('\n')
        .join("package test;", "import android.os.Parcel;", "import android.os.Parcelable;",
            ...
            "private final Integer child;", "public Child(Integer child) {", "this.child = child;",
            "}", "public Integer getChild() {", "return this.child;", "}",
            "public static final Parcelable.Creator<Child> CREATOR = new Parcelable.Creator<Child>() {",
            "@Override public Child createFromParcel(Parcel in) {", "return new Child(in);", "}",
            "@Override public Child[] newArray(int size) {", "return new Child[size];", "}", "};",
            "private Child(Parcel in) {", "child = in.readInt();", "}",
            "@Override public int describeContents() {", "return 0;", "}",
            "@Override public void writeToParcel(Parcel dest, int flags) {",
            "dest.writeInt(child);", "}", "}"));
    ...
    JavaFileObject rootParcel = JavaFileObjects.forSourceString("test/RootParcel", Joiner.on('\n')
        .join("package test;", "import android.os.Parcel;", "import android.os.Parcelable;",
            ...
            "@Override public RootParcel createFromParcel(Parcel in) {", "Child outChild = null;",
            "if (in.readInt() == 0) {", "outChild = Child.CREATOR.createFromParcel(in);", "}",
            "Root data = new Root(outChild);", "return new RootParcel(data);", "}",
            ...
            "@Override public int describeContents() {", "return 0;", "}",
            "@Override public void writeToParcel(Parcel dest, int flags) {",
            "Child child = this.data.getChild();", "if (child == null) {", "dest.writeInt(1);",
            "} else {", "dest.writeInt(0);", "child.writeToParcel(dest, flags);", "}", "}", "}"));
