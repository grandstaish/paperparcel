...
public class QueueTests {
  ...
  @Test public void queueOfParcelableTypesTest() throws Exception {
    JavaFileObject source =
        JavaFileObjects.forSourceString("test.Test", Joiner.on('\n').join(
            ...
            "  private final Queue<Integer> child;",
            "  public Test(Queue<Integer> child) {",
            "    this.child = child;",
            "  }",
            "  public Queue<Integer> getChild() {",
            "    return this.child;",
            "  }",
            "}"
        ));
    ...
    JavaFileObject expectedSource =
        JavaFileObjects.forSourceString("test/Test$$Wrapper", Joiner.on('\n').join(
            ...
            "    Queue<Integer> child = this.data.getChild();",
            "    if (child == null) {",
            "      dest.writeInt(1);",
            "    } else {",
            "      dest.writeInt(0);",
