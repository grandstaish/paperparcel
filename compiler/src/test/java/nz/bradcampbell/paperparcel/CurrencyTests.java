...
public class CurrencyTests {
  ...
  @Test public void currencyTest() throws Exception {
    JavaFileObject source =
        JavaFileObjects.forSourceString("test.Test", Joiner.on('\n').join(
            ...
            "  private final Currency child;",
            "  public Test(Currency child) {",
            "    this.child = child;",
            "  }",
            "  public Currency getChild() {",
            "    return this.child;",
            "  }",
            "}"
        ));
    ...
    JavaFileObject expectedSource =
        JavaFileObjects.forSourceString("test/Test$$Wrapper", Joiner.on('\n').join(
            ...
            "      Currency outChild = null;",
            "      if (in.readInt() == 0) {",
            "        outChild = Currency.getInstance(in.readString());",
            "      }",
            ...
            "    Currency child = this.data.getChild();",
            "    if (child == null) {",
            "      dest.writeInt(1);",
            "    } else {",
            "      dest.writeInt(0);",
            "      dest.writeString(child.getCurrencyCode());",
            "    }",
            "  }",
            "}"
        ));
