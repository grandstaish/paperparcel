    ...
    JavaFileObject expectedSource = JavaFileObjects.forSourceString("test/TestParcel",
        Joiner.on('\n')
            .join("package test;", "import android.os.Parcel;", "import android.os.Parcelable;",
                ...
                "int[] outChild = null;", "if (in.readInt() == 0) {",
                "outChild = in.createIntArray();", "}", "Test data = new Test(outChild);",
                ...
                "int[] child = this.data.getChild();", "if (child == null) {", "dest.writeInt(1);",
                "} else {", "dest.writeInt(0);", "dest.writeIntArray(child);", "}", "}", "}"));
